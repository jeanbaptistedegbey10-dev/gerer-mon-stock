    ...
    try {
      ...
      const margeProduit = (product) => {
        ...
        if (achats.length > 0) {
          ...
          return product.sale_price > 0
            ? (product.sale_price - coutMoyen) / product.sale_price : 0
        }
        return product.purchase_price > 0 && product.sale_price > 0
          ? (product.sale_price - product.purchase_price) / product.sale_price : 0
      }
      ...
      setData({
        ...
        kpis: {
          ...
          margeAffichee:   Math.round(margemoyenne * 100),
        },
        recentSales,
      })
    } catch (err) {
